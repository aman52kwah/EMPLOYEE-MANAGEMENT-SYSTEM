     ...
     const recentHires = employees
        ...
        return (
            <div className="space-y-6">
                ...
                <div className='grid grid-cols-1 lg:grid-cols-2 gap6'>
                {/* Department Overview */}
                <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
                    <h3 className="text-lg font-semibold text-gray-900 mb-4">Department Overiew</h3>
                    <div className="space-y-4">
                        {departmentStats.map((dept)=>(
                            <div key={dept.id} className="flex items-center justify-between py-2 border-b border-gray-100 last:border-b-0">
                                <div>
                                    <p className='font-medium text-gray-900'>{dept.name}</p>
                                    <p className="text-sm text-gray-500">{dept.description}</p>
                                </div>
                                <div className='text-right'>
                                    <p className="font-semibold text-gray-900">{dept.employeeCount} employees</p>
                                    <p className='text-sm text-gray-500'>${(dept.budget /1000).toFixed(0)}K budget</p>
                                </div>
                            </div>
                        ))}
                    </div>
                ...
                {/* Recnt Hires */}
                <div className="bg-white round-lg shadow-sm border border-gray-200 p-6">
                    <h3 className="text-lg font-semibold text-gray-900 mb-4">Recent Hires</h3>
                    <div className="space-y-4">
                        {recentHires.map((employee)=>(
                            <div key={employee.id} className="flex items-center space-x-4 py-2">
                ...
                <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
                    ...
                    <div className="grid grid-cols-1 mdLgrid-cols-3 gap-4">
                        <button className="flex items-center justify-center p-4 border-2 border-dashed border-gray-300 rounded-lg hover:border-blue-500 
